...
class MocapDataHelper(object):
        ...
        for di in data:
            for idx in range(3):
                if data[di][idx] is not None: 
                    ...
                    data[di][idx][0] = self.mocap_pose * data[di][idx][0];
    ...
    def read_poses_data(self, filename, scale_factor = 0.10 ):
        # 0  3     4    3     3    3       4     3      3      3      4      3      3  
        # 0 1:4   4:8  8:11 11:14 14:17  17:21  21:24  24:27  27:30  30:34  34:37 37:40
        # T PosA quatA  VpA  VqA  PosB   quatB   VpB    VqB    PosC  quatC   VpC   VqC 
        # 
        ...
        for line in file:
            ...
            # idx = int(l[0])
            ti = float(l[0])
            t0 = np.array(l[1:4], np.float32) * scale_factor
            q0 = np.array(l[4:8], np.float32)
            v0 = np.array(l[11:14]+l[8:11], np.float32)
            t1 = np.array(l[14:17], np.float32) * scale_factor
            q1 = np.array(l[17:21], np.float32)
            v1 = np.array(l[24:27]+ l[21:24], np.float32)
            t2 = np.array(l[27:30], np.float32) * scale_factor
            q2 = np.array(l[30:34], np.float32)
            v2 = np.array(l[37:40]+ l[34:37], np.float32)

            # print(f"t0 {t0}")
            # print(f"q0 {q0}")
            # print(f"v0 {v0}")
            poses = []
            for q, t, v in zip([q0,q1,q2], [t0,t1,t2], [v0, v1, v2]):
                if np.isnan(q).any() or np.isnan(t).any():
                    poses.append(None)
                else:
                    R = gtsam.Rot3(q[0],q[1],q[2],q[3]);
                    poses.append([gtsam.Pose3(R, t ), v])
    ...
    def compute_errors(self, gt_data ):
        ...
        for gt_ti in gt_data:
            ...
            v0, v1, v2 = self.cheb.compute_velocities(gt_ti)
            poses_gts = gt_data[gt_ti]

            for z, v, gtPV in zip([p0, p1, p2], [v0, v1, v2], poses_gts):
                # print(gtPV)
                if gtPV is not None:
                    gt = gtPV[0]
                    gtv = gtPV[1]
                    ...
                    # print(f"gt_ti {gt_ti} z {z}")
                    # print(f"gt_ti {gt_ti} gt {gt}")
                    # print(f"gt_ti {gt_ti} err {err}")
                    err_v = np.abs(gtv - v)
                    # print(f"v {v}\n gtv {gtv}")
                    err_cm = np.abs(gt.translation() - z.translation())
                    error.append(err)
                    error_v.append(err_v)
                    error_cm.append(err_cm)
                    angles.append(self.angle_err(gt, z));
                else:
                    error.append(np.array([np.nan]*6))
                    error_v.append(np.array([np.nan]*6))
                    error_cm.append(np.array([np.nan]*3))
                    angles.append(np.nan);
                    # print(f"err {err}")
                # error.append(np.abs(gtsam.Pose3.Logmap(gt_p.between(z_p))))
                all_errors.append(error)
                all_errors_v.append(error_v)
                all_errors_cm.append(error_cm)
                angle_errs.append(angles)
        return all_errors, all_errors_v, all_errors_cm, angle_errs

    def compute_mean_stddev(self, all_errors, all_errors_v, all_errors_cm, angle_errs, filename_prefix ):
        ...
        mean_err_v = np.nanmean(all_errors_v, axis=(0,1))
        stdd_err_v = np.nanstd(all_errors_v, axis=(0,1))
        ...
        # print(f"mean_v {mean_err_v}\n stdd_v {stdd_err_v}\n")
        all_errors_file = open(filename_prefix + "_all_errors.txt", 'w');
        all_errors_v_file = open(filename_prefix + "_all_errors_vel.txt", 'w');
        ...
        for errs in all_errors_v:
            for e in errs:
                for xi in e:
                    all_errors_v_file.write(str(xi) + " ")
                all_errors_v_file.write("\n")
...
if __name__ == '__main__':
    ...
    argparse.add_argument('-d', '--directory', help='Estimation', required=True)
    ...
    for idx in range(25, 50):
        ...
        if gt_size == 0 or cheb_size == 0:
            print(f"File [{idx}] has no data")
            continue;

        mdh = MocapDataHelper(cheb_file)
        gt_data = mdh.read_poses_data(gt_file);

        errors, errors_v, errors_cm, angles = mdh.compute_errors(gt_data )
        all_errors += errors
        all_errors_v += errors_v
        all_errors_cm += errors_cm
        angle_errs += angles
    mdh.compute_mean_stddev(all_errors, all_errors_v, all_errors_cm, angle_errs, output_prefix)
