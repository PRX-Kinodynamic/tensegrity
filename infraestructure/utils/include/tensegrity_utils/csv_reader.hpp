...
public:
  ...
  csv_reader_t(const std::string filename, const char separator = ' ')
    : _filename(filename), file(filename.c_str()), _separator(separator)
  {
    TENSEGRITY_ASSERT(std::filesystem::exists(filename), "Filename [" + filename + "] does not exists.");
  }
  ...
  Line<T> next_line()
  {
    ...
    return tensegrity::utils::split<T>(line, _separator);
  }
  ...
  Block<T> read_columns(const Container container)
  {
    ...
    while (has_next_line())
    {
      ...
      for (int i = 0; i < container.size(); ++i)
      {
        ...
        if (line.size() < idx)
          continue;
        columns[i].push_back(tensegrity::utils::convert_to<T>(line[idx]));
      }
    }
    return columns;
  }
