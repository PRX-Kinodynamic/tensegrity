...
int main(int argc, char* argv[])
{
  ...
  while (ros::ok())
  {
    if (node_status.status() == interface::NodeStatus::RUNNING)
    {
      ...
      if (use_cable_sensors and total_constraints >= 15)
      {
        add_cable_meassurements(sensors_callback, idx, graph, values, color_map);
        estimation::add_to_values(values, rod_symbol(RodColors::RED, idx), red_bar);
        estimation::add_to_values(values, rod_symbol(RodColors::GREEN, idx), green_bar);
        estimation::add_to_values(values, rod_symbol(RodColors::BLUE, idx), blue_bar);
        ...
        // add_to_values(values, rotation_symbol(RodColors::RED, idx), Rotation());
        // add_to_values(values, rotation_symbol(RodColors::GREEN, idx), Rotation());
        // add_to_values(values, rotation_symbol(RodColors::BLUE, idx), Rotation());

        // const gtsam::noiseModel::Base::shared_ptr rot_noise{ gtsam::noiseModel::Isotropic::Sigma(3, 1e-5) };

        // const gtsam::Key key_rot_red{ rotation_symbol(RodColors::RED, idx,0) };
        // const gtsam::Key key_rot_green{ rotation_symbol(RodColors::GREEN, idx,0) };
        // const gtsam::Key key_rot_blue{ rotation_symbol(RodColors::BLUE, idx,0) };
