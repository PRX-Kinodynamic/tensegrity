...
int update_fg(const RodObservation& observations, const estimation::RodColors color, gtsam::NonlinearFactorGraph& graph,
              gtsam::Values& values, estimation::observation_update_t& observation_update)
{
  ...
  {
    observation_update.zA = observations.first.value();
    add_one_observation(observation_update, graph, values);
    return 1;
  }
  else if (observations.second)
  {
    observation_update.zA = observations.second.value();
    add_one_observation(observation_update, graph, values);
    return 1;
  }
  return 0;
...
int main(int argc, char* argv[])
{
  ...
  while (ros::ok())
  {
    if (node_status.status() == interface::NodeStatus::RUNNING)
    {
      ...
      const int tot_red{ update_fg(red_observations, estimation::RodColors::RED, graph, values, observation_update) };
      const int tot_green{ update_fg(green_observations, estimation::RodColors::GREEN, graph, values,
                                     observation_update) };
      const int tot_blue{ update_fg(blue_observations, estimation::RodColors::BLUE, graph, values,
                                    observation_update) };

      DEBUG_VARS(tot_red, tot_green, tot_blue);
      if (tot_red + tot_green + tot_blue > 3)
      {
        add_triangle_factors(observation_update, graph, values);

        const gtsam::Key key_Xr{ estimation::rod_symbol(estimation::RodColors::RED, 0, 0) };
        const gtsam::Key key_Xg{ estimation::rod_symbol(estimation::RodColors::GREEN, 0, 0) };
        const gtsam::Key key_Xb{ estimation::rod_symbol(estimation::RodColors::BLUE, 0, 0) };

        estimation::add_to_values(values, key_Xr, factor_graphs::random<SE3>());
        estimation::add_to_values(values, key_Xg, factor_graphs::random<SE3>());
        estimation::add_to_values(values, key_Xb, factor_graphs::random<SE3>());
